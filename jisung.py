...
jisung_bp = Blueprint("jisung_bp", __name__)



pertanyaan = {
    "hai jisung_bp": ["Hai sayang 😘, gimana kamu udah makan belum?"],
...
@jisung_bp.route('/jisung/reset')
def reset():
    session.pop('history', None)
    return render_template("jisung.html", history = [], result=None)


@jisung_bp.route('/jisung', methods=['GET', 'POST'])
def index():
    result = ""
    ...
    return render_template("jisung.html", history = session["history"], result=result)
