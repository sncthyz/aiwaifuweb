...
jiwoo_bp = Blueprint("jiwoo", __name__)



pertanyaan = {
    "hai jiwoo": ["Hai sayang 😘, gimana kamu udah makan belum?"],
...
@jiwoo_bp.route('/jiwoo/reset')
def reset():
    session.pop('history', None)
    return render_template("jiwoo.html", history = [], result=None)


@jiwoo_bp.route('/jiwoo', methods=['GET', 'POST'])
def index():
    result = ""
    ...
    return render_template("jiwoo.html", history = session["history"], result=result)
