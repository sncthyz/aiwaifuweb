...
jeno_bp = Blueprint("jeno", __name__)
...
@jeno_bp.route('/jeno/reset')
def reset():
    session.pop('history', None)
    return render_template("jeno.html", history = [], result=None)


@jeno_bp.route('/jeno', methods=['GET', 'POST'])
def index():
    result = ""
    ...
    return render_template("jeno.html", history = session["history"], result=result)
