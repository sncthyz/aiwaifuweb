...
gojo_bp = Blueprint("gojo", __name__)



pertanyaan = {
    "hai gojo": ["Hai sayang 😘, gimana kamu udah makan belum?"],
...
@gojo_bp.route('/gojo/reset')
def reset():
    session.pop('history', None)
    return render_template("gojo.html", history = [], result=None)


@gojo_bp.route('/gojo', methods=['GET', 'POST'])
def index():
    result = ""
    ...
    return render_template("gojo.html", history = session["history"], result=result)
