...
dazai_bp = Blueprint("alya", __name__)
...
@dazai_bp.route('/dazai/reset')
def reset():
    session.pop('history', None)
    return render_template("dazai.html", history = [], result=None)


@dazai_bp.route('/dazai', methods=['GET', 'POST'])
def index():
    result = ""
    ...
    return render_template("dazai.html", history = session["history"], result=result)
