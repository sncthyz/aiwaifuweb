...
oreki_bp = Blueprint("oreki", __name__)



pertanyaan = {
    "hai Oreki": ["Hai sayang 😘, gimana kamu udah makan belum?"],
...
@oreki_bp.route('/oreki/reset')
def reset():
    session.pop('history', None)
    return render_template("oreki.html", history = [], result=None)


@oreki_bp.route('/oreki', methods=['GET', 'POST'])
def index():
    result = ""
    ...
    return render_template("oreki.html", history = session["history"], result=result)
