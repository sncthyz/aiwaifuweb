...
nanami_bp = Blueprint("nanami", __name__)



pertanyaan = {
    "hai nanami": ["Hai sayang 😘, gimana kamu udah makan belum?"],
...
@nanami_bp.route('/nanami/reset')
def reset():
    session.pop('history', None)
    return render_template("nanami.html", history = [], result=None)


@nanami_bp.route('/nanami', methods=['GET', 'POST'])
def index():
    result = ""
    ...
    return render_template("nanami.html", history = session["history"], result=result)
